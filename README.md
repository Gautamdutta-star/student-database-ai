...
A modular backend application for managing student records with CRUD APIs, an AI-powered chatbot, LangGraph, Google Gemini API, and ChromaDB semantic search.
...
| Google Gemini API | AI and natural language processing |
...
The chatbot can use database tools and semantic search to retrieve relevant student information.
...
> ⚠️ Never commit your actual Gemini API key to GitHub.
...
The API key should be configured securely in the local `.env` file or in the deployment platform environment variables.

> ⚠️ API keys must never be committed to the GitHub repository.
